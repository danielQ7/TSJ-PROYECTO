...
<div class="d-flex align-items-center gap-3 mb-4">
    ...
    <h4 class="fw-bold mb-0">Nuevo Funcionario</h4>
</div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <select name="id_vinculo" class="form-select" required>
                        ...
                        @foreach($vinculos as $v)
                            <option value="{{ $v->id_vinculo }}" {{ old('id_vinculo') == $v->id_vinculo ? 'selected' : '' }}>
                                {{ $v->descripcion }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-4">
                    ...
                    <select name="id_cargo" class="form-select" required>
                        <option value="">Seleccionar...</option>
                        @foreach($cargos as $c)
                            <option value="{{ $c->id_cargo }}" {{ old('id_cargo') == $c->id_cargo ? 'selected' : '' }}>
                                {{ $c->descripcion }}
                            </option>
                        @endforeach
                    </select>
